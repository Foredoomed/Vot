...
static const int kGDataXMLParseOptions =
    (XML_PARSE_NOCDATA | XML_PARSE_NOBLANKS);

// dictionary key callbacks for string cache
static const void *StringCacheKeyRetainCallBack(CFAllocatorRef allocator,
                                                const void *str);
static void StringCacheKeyReleaseCallBack(CFAllocatorRef allocator,
                                          const void *str);
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *fakeQName =
      [NSString stringWithFormat:@"{%@}:%@", theURI, localName];
  return fakeQName;
}
...
- (NSString *)XMLString {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (buff) {
      ...
      if (result > -1) {
        str =
            [[[NSString alloc] initWithBytes:(xmlBufferContent(buff))
  ...
  if (xmlNode_ != NULL && xmlNode_->doc != NULL) {
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        // xpath context
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL;
               nsPtr = nsPtr->next) {
      ...
      if (xpathObj) {
        ...
        if (msg) {
          ...
          errorInfo =
              [NSDictionary dictionaryWithObject:errStr forKey:@"error"];
        }
      }
    ...
    errorInfo =
        [NSDictionary dictionaryWithObject:@"invalid node" forKey:@"error"];
  }
...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"%@ %p: {type:%d name:%@ xml:\"%@\"}",
                                    [self class], self, nodeType, [self name],
                                    [self XMLString]];
}
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *)"xml")) {
        GDataXMLNode *node =
            [GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr)currNS];
...
- (NSArray *)elementsForLocalName:(NSString *)localName URI:(NSString *)URI {
  ...
  if (xmlNode_ != NULL && xmlNode_->children != NULL) {
    ...
    xmlNsPtr foundParentNS =
        xmlSearchNsByHref(xmlNode_->doc, xmlNode_, desiredNSHref);
    ...
    for (GDataXMLNode *child in children) {
      ...
      if (currChildPtr->type == XML_ELEMENT_NODE) {
        ...
        if (currChildPtr->nsDef != NULL) {
          ...
          childLocalNS =
              xmlSearchNsByHref(xmlNode_->doc, currChildPtr, desiredNSHref);
        ...
        // seeking
        if (currChildPtr->ns == childLocalNS && currChildPtr->name != NULL &&
    ...
    if (attrPtr) {
      ...
      if (oldAttr == NULL) {
        ...
        if (attrPtr->ns != NULL) {
          ...
          newPropNS =
              xmlSearchNsByHref(xmlNode_->doc, xmlNode_, attrPtr->ns->href);
          if (newPropNS == NULL) {
            // make a new namespace on the parent node, and use that for the
            // new attribute
            newPropNS =
                xmlNewNs(xmlNode_, attrPtr->ns->href, attrPtr->ns->prefix);
          }
        }
        ...
        xmlAttrPtr newProp =
            xmlNewNsProp(xmlNode_, newPropNS, attrPtr->name, value);
        if (newProp != NULL) {
...
- (GDataXMLNode *)attributeForLocalName:(NSString *)localName
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr == NULL) {
      ...
      NSString *fakeQName =
          GDataFakeQNameForURIAndName(attributeURI, localName);
    ...
    if (localName != NULL) {
      if (prefix != NULL) {
        ...
        if (prefixLen > 2 && prefix[0] == '{' && prefix[prefixLen - 1] == '}') {
          ...
          if (uri != NULL) {
            foundNS =
                xmlSearchNsByHref(graftPointNode->doc, graftPointNode, uri);
...
+ (void)fixUpNamespacesForNode:(xmlNodePtr)nodeToFix
  ...
  // Do the fixes
  [self fixQualifiedNamesForNode:nodeToFix graftingToTreeNode:graftPointNode];
...
- (id)initWithData:(NSData *)data
  ...
  if (self) {
    ...
    xmlDoc_ = xmlReadMemory(
        (const char *)[data bytes], (int)[data length], baseURL, encoding,
...
// utility routine for init methods
  ...
  CFDictionaryKeyCallBacks keyCallBacks = {
    0, // version
    StringCacheKeyRetainCallBack,          StringCacheKeyReleaseCallBack,
    StringCacheKeyCopyDescriptionCallBack, StringCacheKeyEqualCallBack,
    StringCacheKeyHashCallBack
  };

  CFMutableDictionaryRef dict =
      CFDictionaryCreateMutable(kCFAllocatorDefault, capacity, &keyCallBacks,
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
    GDataXMLNode *docNode =
        [GDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
    NSArray *array =
        [docNode nodesForXPath:xpath namespaces:namespaces error:error];
    return array;
  }
  return nil;
}
...
static CFStringRef StringCacheKeyCopyDescriptionCallBack(const void *str) {
  ...
  CFStringRef cfStr = CFStringCreateWithCString(
      kCFAllocatorDefault, (const char *)str, kCFStringEncodingUTF8);
  return cfStr;
}
